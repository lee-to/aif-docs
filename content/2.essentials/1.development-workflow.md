...
description: The end-to-end flow from planning to implementation and review.
navigation:
  icon: i-lucide-git-branch
---

AI Factory runs in two phases: **project configuration** (one-time) and the **development workflow** (repeatable).
...
The loop is predictable and resumable: plan → implement → verify → commit → evolve.
...
For full contracts and state transition rules, see [Reflex Loop](/ai/reflex-loop).
...
For full details on all skills including utility commands (`/aif-docs`, `/aif-dockerize`, `/aif-build-automation`, `/aif-ci`, `/aif-commit`, `/aif-skill-generator`, `/aif-security-checklist`), see [Core Skills](/ai/core-skills).
...
- [Core Skills](/ai/core-skills) — detailed reference for all workflow and utility skills
- [Plan Files](/essentials/plan-files) — how plan artifacts are stored and managed
